...
class KMeans extends KMeansInterface {
  ...
  def classify(points: Seq[Point], means: Seq[Point]): Map[Point, Seq[Point]] = {
    if (points.isEmpty) {
      means.map(a => (a, Seq.empty)).toMap
    } else
    points.map(p => (p, findClosest(p, means)))
      .groupBy(f => f._2).map{case (m, pAndM) => (m, pAndM.map(_._1))}
  }

  def classify(points: ParSeq[Point], means: ParSeq[Point]): ParMap[Point, ParSeq[Point]] = {
    if (points.isEmpty) {
      means.map(a => (a, ParSeq.empty)).toMap
    } else
      points.map(p => (p, findClosest(p, means)))
        .groupBy(f => f._2).map{case (m, pAndM) => (m, pAndM.map(_._1))}
  }
  ...
  final def kMeans(points: Seq[Point], means: Seq[Point], eta: Double): Seq[Point] = {
    ...
    val ifConverged = converged(eta, means, newMeans)
    if (ifConverged) means else kMeans(points, newMeans, eta)
  }
